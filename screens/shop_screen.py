...
def shop_screen():
    while True:
        image = pygame.image.load('data\\screens_foto\\shop.jpg')
        ...
        for i in range(1, 1 + len(skins)):
            if i == skin_id:
                ...
                sprite.image = load_image(f"btns\\skin_on.png")
                ...
                sprite = pygame.sprite.Sprite()
                sprite.image = load_image(f"btns\\{i}not.png")
                sprite.rect = sprite.image.get_rect()
                sprite.rect.x = 340
                sprite.rect.y = 66 + (93 * (i - 1))
                btns.add(sprite)
                ...
                sprite = pygame.sprite.Sprite()
                sprite.image = load_image(f"btns\\{i}have.png")
                sprite.rect = sprite.image.get_rect()
                sprite.rect.x = 340
                sprite.rect.y = 66 + (93 * (i - 1))
                btns.add(sprite)
